#!/usr/bin/env python
"""Validation-only entry point for the marine debris YOLO pipeline.

Run from the ``backend`` directory::

    python validate_model.py                          # validate default model
    python validate_model.py --best-model models/my_model.pt
    python validate_model.py --data-root backend/data --device cpu

Loads the trained model (default ``models/marine_debris_yolov8.pt``) and
computes genuine Precision / Recall / mAP50 / mAP50-95 from real inference over
the VALIDATION split. Requires the split folders to exist (run ``train.py`` or
...
from app.training.evaluate import evaluate_validation
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="validate_model",
        description=(
            "Evaluate the trained YOLO model on the VALIDATION split: real "
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        metrics = evaluate_validation(config)
    except Exception as exc:  # noqa: BLE001 - surface actionable error
        print(f"[validate_model] ERROR: {exc}", file=sys.stderr)
        return 1

    print()
    print("=" * 56)
    print("VALIDATION RESULTS (real inference on the validation split)")
